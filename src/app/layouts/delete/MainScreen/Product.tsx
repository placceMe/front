...
@media (max-width:768px){ .row-title{ font-size:18px; } .row-track {
        -ms-overflow-style: none;  
...
const ProductPage: React.FC = () => {
  ...
  // 1) Делаем стабильную ссылку на request, чтобы эффекты НЕ зависели от "прыгающей" функции
  ...
  // ---- Товары по категориям (дедуп, StrictMode-safe) ----
  ...
  const fetchedOk = useRef<Set<string>>(new Set()); // помечаем только после успешной загрузки
  ...
  useEffect(() => {
    ...
    categories.forEach(cat => {
      if (fetchedOk.current.has(cat.id)) return;           // уже успешно загружали
      if (catsLoading[cat.id] || catProducts[cat.id]?.length) return; // уже в процессе или есть данные
    ...
    // Критично: завись только от набора категорий, а не от request/локальных стейтов
